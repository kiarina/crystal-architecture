...
### 2.4. 公開 API はサブパッケージのトップレベルに集約する
...
ただし、定義した複数のスキーマやモデルの Union 型を定義する場合に、
_types から _schemas や _models を参照するといったケースは、例外的に許容します。
...
`AgentName` のような型エイリアスは、無条件に増やすものではありません。**値が代入・使用される地点で意味を失うとき**にだけ導入します。判断の目安は次の 2 つです。

* **生の型だけでは意味が伝わらない。** `str` や `tuple[str, str]` を見ても「何の文字列か」が分からず、コメントで補わざるを得ないもの。
* **短いローカルスコープに閉じていない。** フィールド・辞書のキー・インデックスの型（例: `dict[tuple[AgentName, AgentKey], Agent]`）は宣言から遠く離れたファイルの各所に現れ、出自が希薄になります。こうした値こそ型で意味を運ぶ価値があります。直前の行から意味が自明なローカル変数には不要です。

型エイリアスの役割は「**スコープを越えて意味を運ぶこと**」です。本来コメントで補っていた意味を型へ昇格させる、と捉えてください。逆に、インラインで読んで意味が通る値（`priority: int` など）にまで型を切るのは過剰です。

**docstring は定義時に書く:**

型やフィールドに docstring を付けるなら、定義の直後（代入の次行）に書くことを推奨します。IDE の補完やホバーでその記号自身の説明として表示されるためです。
...
ただし、型エイリアス側で意味を説明しているフィールドには、フィールド側の docstring を重ねて書きません。重複するためです。

### 2.7. コメント・docstring は最小限にする

コメントや docstring は、**そのコード・型・名前・ファイル内の記述を読めば分かること**には書きません。書くのは、別のファイルやモジュールを参照しないと分からない情報——設計上の意図、スレッドや実行順序の制約、外部とのプロトコル、呼び出し側との契約——に限定します。

```python
# Avoid: 本文を読めば分かることの言い換え
def resolve_content_type(filename: str, content_type: str | None) -> str:
    """明示があればそれを使い、なければ拡張子から推測し、最後は既定値を返す。"""
    return content_type or guess_type(filename)[0] or DEFAULT_CONTENT_TYPE
```

```python
# Good: 本文からは読み取れない、呼び出し側の制約だけを残す
def sweep_expired(self) -> list[str]:
    """ワーカースレッド上で呼ぶこと（フレームワークのメモリを解放するため、
    イベントループから呼んではならない）。"""
    ...
```

さらに、残すべき情報も可能な限り **型や名前で表現して、文章を減らします**。型エイリアス（原則2.6）や、意味のある引数名・関数名で表現できるなら、docstring を書く前にそちらを優先します。docstring は最後の手段です。

## 3. 単一責任と分割のサイン

「1ファイル1責務」と「1サブパッケージ1責務」。
これらは一見すると矛盾しているように感じるかもしれませんが、**粒度が違うだけでどちらも守ることが重要**です。

1つのファイルが持つべき責務は単一であり、同様に1つのサブパッケージが持つべき機能（ドメインの境界）も単一であるべきです。

この「単一責任」が崩れてくると、プロジェクトに以下のような兆候が現れます。

* しっくりくる、明確なファイル名やパッケージ名が決まらない
* サブパッケージ内のファイルツリーが肥大化し、見た目が美しくない
* サブパッケージ内、あるいはサブパッケージ間の依存関係が複雑に絡み合っている

これらの兆候は、そのファイルが複数の責務を持ちすぎているか、サブパッケージが複数の機能を抱え込んでいる証拠です。
これらを**ファイル分割やサブパッケージ分割の明確なサイン**として捉え、リファクタリングを行います。
